// ══════════════════════════════════════════════════════════════
// ENERGY SYSTEM — Core (auth, DB, vues, roster, illustration)
// ══════════════════════════════════════════════════════════════
...
async function doLogout() { toggleUserMenu(false); await sb.auth.signOut(); }
...
async function unfollowChar(charId) {
  await sb.from('followed_characters').delete().eq('user_id', currentUser.id).eq('character_id', charId);
...
async function init() {
  ...
  try {
    ...
    if (session?.user) await onSignedIn(session.user); else onSignedOut();
  } catch(e) { clearTimeout(safetyTimer); onSignedOut(); }
...
function setIllusPreview(url, position) {
  ...
  if (url) {
    img.src = url; img.style.display = 'block'; img.style.objectPosition = `center ${pos}%`;
    ...
    zone.classList.remove('has-image'); sliderWrap.classList.remove('visible'); slider.value = 0;
  }
}
...
async function removeIllustration() {
  ...
  state.illustration_url = ''; state.illustration_position = 0;
  setIllusPreview('', 0); updatePreview();
}
...
function setSaveIndicator(st, msg) {
  ...
  el.textContent = msg; el.className = `save-indicator show ${st}`;
